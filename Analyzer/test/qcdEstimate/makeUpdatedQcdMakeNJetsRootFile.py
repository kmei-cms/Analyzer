#This is the script that takes the QCD.root file produced by the MakeNJets analyzer and creates the new QCD.root updated file that uses the data in the control region as the actual estimate for QCD
...
def main() :
    ...
    makeNjetsForAronFileName    = yearTag+"_QCD.root"
    makeNjetsForAronFile        = ROOT.TFile.Open( makeNjetsForAronFileName )

    qcdSrAllNjetsHisto          = makeNjetsForAronFile.Get( "h_njets_pt30_1l" )
    ...
    for binIter in xrange( 1, qcdCrMCShapeHistoDict["All"].GetNbinsX() + 1) :
        print qcdCrMCShapeHistoDict["All"].GetBinError(binIter), qcdSrAllNjetsHisto.GetBinError(binIter)
    ...
    print math.sqrt(crIntegralValueErrSq),math.sqrt(srIntegralValueErrSq),math.sqrt(crIntegralValueErrSq/(qcdCrMCIntegralValues["All"]**2)+srIntegralValueErrSq/(qcdSrAllNjetsIntegralValue**2))
    ...
    for iBin in mvaBinList :
        for iShift in shiftList :
            ...
            for iTag in tagList :
                ...
                for iNjBin in xrange( 1, oldQcdSysHisto.GetNbinsX()+1 ) :

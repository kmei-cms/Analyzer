...
void ExploreEventSelection::Loop(double weight, int maxevents, std::string type, std::string filetag, bool isQuiet)
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) 
   {
      ...
      for ( unsigned int rji=0; rji < Jets->size() ; rji++ ) {
          ...
          if ( !Jets_ID->at(rji)) continue; //If the jet does not pass identification
          if ( jlv.Pt() > 30 )
          { 
              rec_njet_pt30++;
      ...
      std::vector<TLorentzVector> rec_lepton_pt30;
      std::vector<int> rec_charge_muon_pt30;
      std::vector<int> rec_charge_lepton_pt30;
      for (unsigned int imu = 0; imu < Muons->size(); ++imu)
      {
          ...
          if( abs(lvmu.Eta()) < 2.4 && lvmu.Pt() > 30 && Muons_passIso->at(imu))
          {
              ...
              rec_lepton_pt30.push_back(lvmu);
              rec_charge_muon_pt30.push_back(Muons_charge->at(imu));
              rec_charge_lepton_pt30.push_back(Muons_charge->at(imu));
          }
      }
      ...
      for (unsigned int iel = 0; iel < Electrons->size(); ++iel)
      {
          ...
          if( abs(lvel.Eta()) < 2.4 && lvel.Pt() > 30 && Electrons_tightID->at(iel) && Electrons_passIso->at(iel))
          {
              ...
              rec_lepton_pt30.push_back(lvel);
              rec_charge_electron_pt30.push_back(Electrons_charge->at(iel));
              rec_charge_lepton_pt30.push_back(Electrons_charge->at(iel));
          }
      }
      ...
      //Add the requirement that leptons be opposite sign for baseline for 2 leptons

      if( nleptons == 2 ) {
        int totalCharge = rec_charge_lepton_pt30[0] * rec_charge_lepton_pt30[1];
        if( totalCharge > 0 ) passBaseline2l = false;
      }
      if(type == "Data")
      {
      ...
      //Require two hadronic top tags
      passBaseline0l = passBaseline0l && tops.size() >= 2;

      bool pass_g1b = rec_njet_pt30_btag >= 1;
      bool pass_g2b = rec_njet_pt30_btag >= 2;
      ...
      double mbl1 = -1;
      double mblmet1 = -1;
      TLorentzVector used_bjet1;

      double mbl2 = -1;
      double mblmet2 = -1;
      TLorentzVector used_bjet2;
      if(nleptons == 2 && pass_g2b && passBaseline2l) {
        TLorentzVector mylepton1 = rec_lepton_pt30[0];
        TLorentzVector mylepton2 = rec_lepton_pt30[1];

        for( TLorentzVector myb : rec_bjets_pt30 )
        {
            double mass_bl1 = (mylepton1 + myb).M();
            double mass_blmet1 = (mylepton1 + myb + metlv).M();
            double mass_bl2 = (mylepton2 + myb).M();
            double mass_blmet2 = (mylepton2 + myb + metlv).M();
            ...
            else if ( abs(mass_bl1 - 172.5) < abs(mbl1-172.5) ) {
                mbl1 = mass_bl1;
                mblmet1 = mass_blmet1;
                used_bjet1 = myb;
            }
            ...
            else if ( abs(mass_bl2 - 172.5) < abs(mbl2-172.5) ) {
                mbl2 = mass_bl2;
                mblmet2 = mass_blmet2;
                used_bjet2 = myb;
            }
      ...
      bool pass_mbl_double = mbl1 > 30 && mbl1 < 180 && mbl2 > 30 && mbl2 < 180;
      passBaseline2l = pass_mbl_double && passBaseline2l;

...
void AnalyzeEventSelection::Loop(NTupleReader& tr, double weight, int maxevents, std::string filetag, bool isQuiet)
{
    ...
    while( tr.getNextEvent() )
    {
        ...
        my_efficiencies["event_sel_total"]->Fill(HT_trigger>500 && rec_njet_pt45>=6 && NBJets_pt45>0 && tops.size()>0 ,4);
        my_efficiencies["event_sel_total"]->Fill(HT_trigger>500 && rec_njet_pt45>=6 && NBJets_pt45>0 && tops.size()>0 && NBJets_pt45>1 ,5);
        my_efficiencies["event_sel_total"]->Fill(HT_trigger>500 && rec_njet_pt45>=6 && NBJets_pt45>0 && tops.size()>0 && NBJets_pt45>1 && tops.size()>1 ,6);
        my_efficiencies["event_sel_total"]->Fill(HT_trigger>500 && rec_njet_pt45>=6 && NBJets_pt45>0 && tops.size()>0 && NBJets_pt45>1 && tops.size()>1 && rec_njet_pt30>=8 ,7);

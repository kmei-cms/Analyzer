...
void Semra_Analyzer::InitHistos(const std::map<std::string, bool>& cutmap) // define variable map
{
    ...
    my_efficiencies.emplace("event_sel_weight_3t", std::make_shared<TEfficiency>("event_sel_weight_3t","event_sel_weight_3t",9,0,9));
    my_efficiencies.emplace("event_sel_weight_ge3t", std::make_shared<TEfficiency>("event_sel_weight_ge3t","event_sel_weight_ge3t",9,0,9)); 
...
void Semra_Analyzer::Loop(NTupleReader& tr, double weight, int maxevents, bool isQuiet)
{
    ...
    while( tr.getNextEvent() )
    {
        ...
        const auto& pass_ge2t3j     = ntops >= 2 && ntops_3jet >= 2;
